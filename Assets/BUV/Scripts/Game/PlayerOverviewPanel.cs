    ...
    public class PlayerOverviewPanel : MonoBehaviourPunCallbacks
    {
        ...
        public void Awake()
        {
            ...
            foreach (Player p in PhotonNetwork.PlayerList)
            {
                ...
                entry.GetComponent<Text>().color = BUVGame.GetColor(p.GetPlayerNumber());
                entry.GetComponent<Text>().text = string.Format("{0}\nScore: {1}\nLives: {2}", p.NickName, p.GetScore(), BUVGame.PLAYER_MAX_LIVES);
        ...
        public override void OnPlayerPropertiesUpdate(Player targetPlayer, Hashtable changedProps)
        {
            ...
            {
                entry.GetComponent<Text>().text = string.Format("{0}\nScore: {1}\nLives: {2}", targetPlayer.NickName, targetPlayer.GetScore(), targetPlayer.CustomProperties[BUVGame.PLAYER_LIVES]);
            }
        }

// --------------------------------------------------------------------------------------------------------------------
// <copyright file="BUVGameManager.cs" company="Exit Games GmbH">
...
using Photon.Pun;
    public class BUVGameManager : MonoBehaviourPunCallbacks
    {
        public static BUVGameManager Instance = null;
        ...
        public void Start()
        {
            ...
            {
                {BUVGame.PLAYER_LOADED_LEVEL, true}
            };
        ...
        private IEnumerator SpawnAsteroid()
        {
            while (true)
            {
                yield return new WaitForSeconds(Random.Range(BUVGame.ASTEROIDS_MIN_SPAWN_TIME, BUVGame.ASTEROIDS_MAX_SPAWN_TIME));
        ...
        {
            if (changedProps.ContainsKey(BUVGame.PLAYER_LIVES))
            {
                CheckEndOfGame();
                return;
            }
            ...
            if (changedProps.ContainsKey(BUVGame.PLAYER_LOADED_LEVEL))
            {
        ...
        private bool CheckAllPlayerLoadedLevel()
        {
            foreach (Player p in PhotonNetwork.PlayerList)
            {
                ...
                if (p.CustomProperties.TryGetValue(BUVGame.PLAYER_LOADED_LEVEL, out playerLoadedLevel))
                {
        ...
        private void CheckEndOfGame()
        {
            ...
            {
                object lives;
                if (p.CustomProperties.TryGetValue(BUVGame.PLAYER_LIVES, out lives))
                {
                    if ((int) lives > 0)
                    {
                        allDestroyed = false;
